...
const inp = { display:'block', width:'100%', padding:'12px 14px', border:`1.5px solid ${C.border}`, borderRadius:12, fontSize:15, color:C.cream, background:'rgba(245,240,232,0.06)', fontFamily:'inherit', marginBottom:16, boxSizing:'border-box' };

function Label({ children }) {
  return <div style={{ fontSize:12, fontWeight:500, color:C.muted, letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:8 }}>{children}</div>;
}

function Tag({ t }) {
  return <span style={{ background:'rgba(245,240,232,0.08)', color:C.sand, fontSize:12, fontWeight:500, padding:'3px 10px', borderRadius:20 }}>{t}</span>;
}

function Avatar({ src, size=36, border }) {
  const [err, setErr] = useState(false);
  return (
    <div style={{ width:size, height:size, borderRadius:'50%', overflow:'hidden', border: border||`2px solid ${C.border}`, flexShrink:0, background:C.surface }}>
      {!err && <img src={src} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }} onError={()=>setErr(true)} />}
    </div>
  );
}

function LogoMark({ width=130 }) {
  return (
    <svg width={width} viewBox="0 0 130 38" fill="none" xmlns="http://www.w3.org/2000/svg">
      <circle cx="12" cy="19" r="11" fill="#C4622D"/>
      <circle cx="21" cy="19" r="11" fill="#F5F0E8" fillOpacity="0.1" stroke="#F5F0E8" strokeWidth="0.8"/>
      <text x="37" y="26" fontFamily="'Cormorant Garamond', Georgia, serif" fontSize="26" fontWeight="300" letterSpacing="-1.5" fill="#F5F0E8">yumlist</text>
    </svg>
  );
}
...
export default function App() {
  useEffect(() => {
    ...
    lk.href = 'https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,300;0,400;1,300;1,400&family=DM+Sans:opsz,wght@9..40,300;9..40,400;9..40,500&display=swap';
    ...
    st.textContent = `
      *{box-sizing:border-box}
      ...
      input:focus,textarea:focus{outline:none;border-color:rgba(245,240,232,0.4)!important}
  ...
  const doLogin = () => {
    const u = { id:'me', username: form.username||'you', name: form.username||'You',
      ...
      avatar:`https://ui-avatars.com/api/?name=${encodeURIComponent(form.username||'You')}&background=C4622D&color=fff&size=200`,
  ...
  // ── AUTH ──────────────────────────────────────────────
  if (!me) return (
    <div style={{ minHeight:'100vh', background:C.bg, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'32px 24px', fontFamily:"'DM Sans',sans-serif" }}>
      <div className="fin" style={{ textAlign:'center', marginBottom:52 }}>
        <div style={{ display:'flex', justifyContent:'center', marginBottom:16 }}>
          <LogoMark width={150} />
        </div>
        <div style={{ fontSize:14, color:C.sand, marginTop:4, fontWeight:300, letterSpacing:'0.04em' }}>discover through people you trust</div>
      </div>

      <div className="fin" style={{ width:'100%', maxWidth:360, background:C.card, borderRadius:24, padding:'32px 28px', boxShadow:'0 4px 40px rgba(0,0,0,0.5)', animationDelay:'0.1s' }}>
        <h2 style={{ fontSize:20, fontWeight:500, color:C.cream, margin:'0 0 24px', letterSpacing:'-0.3px' }}>
        ...
        {authErr && <div style={{ fontSize:13, color:C.sand, marginBottom:14 }}>{authErr}</div>}
        <button onClick={authMode==='login'?doLogin:doSignup} style={{ display:'block', width:'100%', padding:'14px', background:C.cream, color:C.ink, border:'none', borderRadius:12, fontSize:15, fontWeight:500, cursor:'pointer', marginTop:18 }}>
          {authMode==='login' ? 'Sign in' : 'Create account'}
        </button>
        <div style={{ textAlign:'center', marginTop:18, fontSize:14, color:C.sand }}>
          {authMode==='login' ? "New here? " : "Already have an account? "}
          <span onClick={()=>{setAuthMode(m=>m==='login'?'signup':'login');setAuthErr('');}} style={{ color:C.cream, fontWeight:500, cursor:'pointer' }}>
            {authMode==='login' ? 'Sign up' : 'Sign in'}
          </span>
        </div>
        {authMode==='login' && (
          <div style={{ textAlign:'center', marginTop:24, paddingTop:20, borderTop:`1px solid ${C.border}` }}>
            <div style={{ fontSize:12, color:C.muted, marginBottom:10 }}>Just want to explore?</div>
            <button onClick={doLogin} style={{ padding:'10px 28px', background:C.surface, color:C.sand, border:'none', borderRadius:10, fontSize:14, fontWeight:500, cursor:'pointer' }}>
              Enter as guest →
            </button>
          </div>
        )}
      ...
      <div className="fin" style={{ marginTop:32, display:'flex', alignItems:'center', gap:8, opacity:0.6, animationDelay:'0.2s' }}>
        ...
        <span style={{ fontSize:13, color:C.sand }}>5 people waiting for you</span>
      </div>
    </div>
  );
  ...
  // ── BOTTOM NAV ────────────────────────────────────────
  const BNav = () => (
    <div style={{ background:C.card, borderTop:`1px solid ${C.border}`, display:'flex', alignItems:'center', justifyContent:'space-around', padding:'10px 16px 20px', flexShrink:0 }}>
      <NavBtn icon={<Home size={22} strokeWidth={scr==='feed'?2.2:1.6}/>} label="Home" active={scr==='feed'} onClick={()=>setScr('feed')} />
      <button onClick={()=>{setStep(1);setScr('create');}} style={{ width:50, height:50, borderRadius:'50%', background:C.cream, border:'none', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', boxShadow:'0 4px 20px rgba(0,0,0,0.4)', transform:'translateY(-10px)' }}>
        <Plus size={22} color={C.ink} strokeWidth={2.5} />
      </button>
      ...
      <button onClick={()=>goProfile('me')} style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:3, background:'none', border:'none', cursor:'pointer', padding:'4px 12px' }}>
        <div style={{ width:28, height:28, borderRadius:'50%', overflow:'hidden', border: scr==='profile'?`2.5px solid ${C.cream}`:`2px solid transparent`, background:C.surface, flexShrink:0 }}>
          <img src={me.avatar} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }} onError={()=>{}} />
        </div>
        <span style={{ fontSize:10, color:scr==='profile'?C.cream:C.muted, fontWeight:scr==='profile'?500:400 }}>Me</span>
      </button>
    </div>
  );
  ...
  if (scr === 'create') return (
    <div className="sup" style={{ height:'100vh', background:C.bg, display:'flex', flexDirection:'column', fontFamily:"'DM Sans',sans-serif", maxWidth:430, margin:'0 auto' }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'20px 20px 16px', borderBottom:`1px solid ${C.border}`, flexShrink:0 }}>
        <button onClick={()=>setScr('feed')} style={{ background:'none', border:'none', cursor:'pointer', padding:4 }}><X size={22} color={C.cream}/></button>
        <div style={{ fontSize:16, fontWeight:500, color:C.cream }}>{step===1?'Choose a photo':'Add details'}</div>
        {step===1
          ? <button onClick={()=>draft.img&&setStep(2)} style={{ fontSize:15, fontWeight:500, color:draft.img?C.cream:C.muted, background:'none', border:'none', cursor:draft.img?'pointer':'default' }}>Next →</button>
          : <button onClick={publish} disabled={!draft.place} style={{ fontSize:15, fontWeight:500, color:draft.place?C.cream:C.muted, background:'none', border:'none', cursor:draft.place?'pointer':'default' }}>Post</button>}
      </div>

      {step===1 && (
        <div className="noscr" style={{ flex:1, overflowY:'auto', padding:16 }}>
          {draft.img && <img src={draft.img} alt="" style={{ width:'100%', height:200, objectFit:'cover', borderRadius:16, marginBottom:16, border:`3px solid ${C.cream}` }}/>}
          <div style={{ fontSize:13, color:C.muted, marginBottom:12 }}>Tap a photo to select it</div>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:8 }}>
            {GALLERY.map((img,i)=>(
              <div key={i} onClick={()=>setDraft(d=>({...d,img}))} style={{ aspectRatio:'1', borderRadius:12, overflow:'hidden', cursor:'pointer', border:draft.img===img?`3px solid ${C.cream}`:'3px solid transparent', position:'relative' }}>
                <img src={img} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }}/>
                {draft.img===img && <div style={{ position:'absolute', top:6, right:6, width:22, height:22, background:C.cream, borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center' }}><Check size={13} color={C.ink} strokeWidth={2.5}/></div>}
              </div>
            ))}
          </div>
        </div>
      ...
      {step===2 && (
        <div className="noscr" style={{ flex:1, overflowY:'auto', padding:16 }}>
          ...
          <div style={{ fontSize:12, color:C.muted, textAlign:'right', marginBottom:20 }}>{draft.caption.length}/200</div>
          <Label>Rating</Label>
          <div style={{ display:'flex', gap:6, marginBottom:20 }}>
            {[5,4,3,2,1].map(v=>{
              const r=RATE[v];
              return (
                <button key={v} onClick={()=>setDraft(d=>({...d,rating:v}))} style={{ flex:1, padding:'10px 4px', borderRadius:12, border:draft.rating===v?`2px solid ${C.cream}`:`2px solid ${C.border}`, background:draft.rating===v?C.surface:C.card, cursor:'pointer', display:'flex', flexDirection:'column', alignItems:'center', gap:3 }}>
          ...
          <div style={{ display:'flex', flexWrap:'wrap', gap:8, marginBottom:40 }}>
            {TAGS.map(t=>{
              ...
              return (
                <button key={t} onClick={()=>setDraft(d=>({...d,tags:sel?d.tags.filter(x=>x!==t):[...d.tags,t]}))} style={{ padding:'7px 14px', borderRadius:20, border:sel?`2px solid ${C.cream}`:`2px solid ${C.border}`, background:sel?C.surface:C.card, color:sel?C.cream:C.muted, fontSize:13, fontWeight:sel?500:400, cursor:'pointer' }}>
                  {t}
                </button>
              );
            })}
  ...
  // ── FEED ──────────────────────────────────────────────
  if (scr === 'feed') return (
    <div style={{ height:'100vh', background:C.bg, display:'flex', flexDirection:'column', maxWidth:430, margin:'0 auto', fontFamily:"'DM Sans',sans-serif" }}>
      <div style={{ padding:'16px 20px 12px', borderBottom:`1px solid ${C.border}`, background:C.bg, flexShrink:0 }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <LogoMark width={120} />
          <div style={{ fontSize:12, color:C.sand, background:C.surface, padding:'5px 12px', borderRadius:20 }}>
        ...
        {feedPosts.length===0 ? (
          <div style={{ padding:'24px 0' }}>
            <div style={{ textAlign:'center', marginBottom:28 }}>
              ...
              <div style={{ fontSize:18, fontWeight:500, color:C.cream, marginBottom:6 }}>Follow people to see their places</div>
              <div style={{ fontSize:14, color:C.sand }}>Your feed is personal and chronological</div>
            </div>
            <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
              {USERS.map(u=>(
                <div key={u.id} style={{ background:C.card, borderRadius:16, padding:'14px 16px', display:'flex', alignItems:'center', gap:12, boxShadow:'0 1px 8px rgba(0,0,0,0.2)' }}>
                  ...
                  <div style={{ flex:1, cursor:'pointer' }} onClick={()=>goProfile(u.id)}>
                    <div style={{ fontSize:15, fontWeight:500, color:C.cream }}>{u.name}</div>
                    <div style={{ fontSize:13, color:C.sand }}>@{u.username} · {u.posts} places</div>
                  </div>
                  <button onClick={()=>toggleFollow(u.id)} style={{ padding:'8px 16px', borderRadius:20, border:`1.5px solid ${C.cream}`, background:following.includes(u.id)?'transparent':C.cream, color:following.includes(u.id)?C.cream:C.ink, fontSize:13, fontWeight:500, cursor:'pointer', flexShrink:0 }}>
          ...
          if(!u) return null;
          return (
            <div key={p.id} className="fin" style={{ animationDelay:`${i*0.05}s`, background:C.card, borderRadius:20, overflow:'hidden', marginBottom:16, boxShadow:'0 1px 12px rgba(0,0,0,0.2)' }}>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'13px 14px 10px' }}>
                <div onClick={()=>goProfile(p.uid)} style={{ display:'flex', alignItems:'center', gap:10, cursor:'pointer' }}>
                  ...
                  <div>
                    <div style={{ fontSize:14, fontWeight:500, color:C.cream, lineHeight:1.2 }}>{u.name}</div>
                    <div style={{ fontSize:12, color:C.sand }}>@{u.username}</div>
                  </div>
                </div>
                <div style={{ fontSize:12, color:C.muted }}>{ago(p.at)}</div>
              </div>

              <div style={{ position:'relative' }}>
                <img src={p.img} alt={p.place} style={{ width:'100%', height:260, objectFit:'cover', display:'block' }} onError={e=>e.target.src=GALLERY[0]}/>
                <div style={{ position:'absolute', bottom:10, right:10, background:'rgba(14,13,12,0.75)', backdropFilter:'blur(8px)', borderRadius:20, padding:'5px 10px', display:'flex', alignItems:'center', gap:5 }}>
                  <span style={{ fontSize:15 }}>{r.e}</span>
                  <span style={{ fontSize:12, fontWeight:500, color:C.cream }}>{r.l}</span>
                </div>
              </div>

              <div style={{ padding:'13px 14px 14px' }}>
                <div className="serif" style={{ fontSize:26, fontWeight:300, color:C.cream, lineHeight:1.15, marginBottom:4 }}>{p.place}</div>
                <div style={{ display:'flex', alignItems:'center', gap:4, fontSize:13, color:C.sand, marginBottom:10 }}>
                ...
                {p.caption && <div style={{ fontSize:14, color:C.sand, lineHeight:1.55, marginBottom:12 }}>{p.caption}</div>}
                <div style={{ display:'flex', justifyContent:'flex-end' }}>
                  <button onClick={()=>toggleSave(p.id)} style={{ background:'none', border:'none', cursor:'pointer', padding:'4px 6px', display:'flex', alignItems:'center', gap:5 }}>
                    <Bookmark size={20} strokeWidth={1.8} fill={sv?C.cream:'none'} color={sv?C.cream:C.muted}/>
                    {sv && <span style={{ fontSize:12, color:C.cream, fontWeight:500 }}>Saved</span>}
                  </button>
                </div>
              </div>
            </div>
  ...
  if (scr === 'profile') {
    ...
    return (
      <div style={{ height:'100vh', background:C.bg, display:'flex', flexDirection:'column', maxWidth:430, margin:'0 auto', fontFamily:"'DM Sans',sans-serif" }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'16px 20px', borderBottom:`1px solid ${C.border}`, background:C.bg, flexShrink:0 }}>
          {!isOwn
            ? <button onClick={()=>setScr('feed')} style={{ background:'none', border:'none', cursor:'pointer', padding:4 }}><ChevronLeft size={24} color={C.cream}/></button>
            : <div style={{ width:32 }}/>}
          <div style={{ fontSize:15, fontWeight:500, color:C.cream }}>@{pu.username}</div>
          {isOwn
        ...
        <div className="noscr" style={{ flex:1, overflowY:'auto' }}>
          <div style={{ padding:'28px 20px 20px', textAlign:'center' }}>
            ...
            <div style={{ marginTop:14, fontSize:20, fontWeight:500, color:C.cream }}>{pu.name}</div>
            {pu.bio && <div style={{ fontSize:14, color:C.sand, lineHeight:1.5, maxWidth:260, margin:'6px auto 18px' }}>{pu.bio}</div>}
            <div style={{ display:'flex', justifyContent:'center', gap:40, marginBottom:20 }}>
              {[{v:pposts.length,l:'places'},{v:pu.followers||0,l:'followers'},{v:isOwn?following.length:(pu.following||0),l:'following'}].map(s=>(
                <div key={s.l} style={{ textAlign:'center' }}>
                  <div style={{ fontSize:20, fontWeight:500, color:C.cream, lineHeight:1 }}>{s.v}</div>
                  <div style={{ fontSize:12, color:C.sand, marginTop:3 }}>{s.l}</div>
                </div>
              ))}
            </div>
            {!isOwn && (
              <button onClick={()=>toggleFollow(viewUid)} style={{ padding:'10px 32px', borderRadius:24, border:`1.5px solid ${C.cream}`, background:isF?'transparent':C.cream, color:isF?C.cream:C.ink, fontSize:14, fontWeight:500, cursor:'pointer' }}>
          ...
          {isOwn && (
            <div style={{ display:'flex', borderBottom:`1px solid ${C.border}` }}>
              {['posts','saved'].map(t=>(
                <button key={t} onClick={()=>setProfTab(t)} style={{ flex:1, padding:'12px', background:'none', border:'none', borderBottom:profTab===t?`2px solid ${C.cream}`:'2px solid transparent', color:profTab===t?C.cream:C.muted, fontSize:14, fontWeight:500, cursor:'pointer', textTransform:'capitalize' }}>
                  {t}
                </button>
              ))}
            </div>
          ...
          <div style={{ padding:14, display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
            {disp.length===0 ? (
              <div style={{ gridColumn:'1/-1', textAlign:'center', padding:'40px 20px', color:C.sand, fontSize:14 }}>
                {isOwn&&profTab==='saved' ? 'Tap 🔖 on any post to save it' : 'No places shared yet'}
              </div>
            ) : disp.map(p=>(
              <div key={p.id} style={{ borderRadius:14, overflow:'hidden', background:C.card, boxShadow:'0 1px 8px rgba(0,0,0,0.2)' }}>
                <img src={p.img} alt="" style={{ width:'100%', height:130, objectFit:'cover' }} onError={e=>e.target.src=GALLERY[0]}/>
                <div style={{ padding:'8px 10px 10px' }}>
                  <div className="serif" style={{ fontSize:15, fontWeight:300, color:C.cream, lineHeight:1.2 }}>{p.place}</div>
                  <div style={{ fontSize:11, color:C.sand, marginTop:2 }}>{p.loc}</div>
                  <div style={{ fontSize:11, color:C.muted, marginTop:3 }}>{RATE[p.rating]?.e} {RATE[p.rating]?.l}</div>
                </div>
              </div>
            ))}
          </div>
  ...
  if (scr === 'saved') {
    ...
    return (
      <div style={{ height:'100vh', background:C.bg, display:'flex', flexDirection:'column', maxWidth:430, margin:'0 auto', fontFamily:"'DM Sans',sans-serif" }}>
        <div style={{ padding:'18px 20px 14px', borderBottom:`1px solid ${C.border}`, background:C.bg, flexShrink:0 }}>
          <div className="serif" style={{ fontSize:28, fontWeight:300, color:C.cream, letterSpacing:'-0.5px' }}>Saved</div>
          <div style={{ fontSize:13, color:C.sand, marginTop:2 }}>{sp.length} place{sp.length!==1?'s':''}</div>
        </div>
        ...
        <div className="noscr fin" style={{ flex:1, overflowY:'auto', padding:14 }}>
          {sp.length===0 ? (
            <div style={{ textAlign:'center', padding:'80px 20px' }}>
              ...
              <div style={{ fontSize:17, fontWeight:500, color:C.cream, marginBottom:8 }}>Nothing saved yet</div>
              <div style={{ fontSize:14, color:C.sand, lineHeight:1.6, maxWidth:230, margin:'0 auto' }}>Tap the bookmark icon on any post to save it</div>
            </div>
          ) : (
              ...
              {sp.map(p=>{
                ...
                return (
                  <div key={p.id} style={{ borderRadius:14, overflow:'hidden', background:C.card, boxShadow:'0 1px 8px rgba(0,0,0,0.2)' }}>
                    <div style={{ position:'relative' }}>
                      <img src={p.img} alt="" style={{ width:'100%', height:130, objectFit:'cover' }} onError={e=>e.target.src=GALLERY[0]}/>
                      <button onClick={()=>toggleSave(p.id)} style={{ position:'absolute', top:8, right:8, width:28, height:28, background:'rgba(14,13,12,0.7)', border:'none', borderRadius:'50%', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}>
                        <X size={14} color={C.cream}/>
                      </button>
                    </div>
                    <div style={{ padding:'8px 10px 10px' }}>
                      <div className="serif" style={{ fontSize:15, fontWeight:300, color:C.cream }}>{p.place}</div>
                      <div style={{ fontSize:11, color:C.sand, marginTop:2 }}>{p.loc}</div>
                      <div style={{ fontSize:11, color:C.muted, marginTop:4 }}>by @{u?.username}</div>
                    </div>
                  </div>
                );
              })}
...
function NavBtn({ icon, label, active, onClick }) {
  return (
    <button onClick={onClick} style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:3, background:'none', border:'none', cursor:'pointer', padding:'4px 12px' }}>
      <div style={{ color: active?'#F5F0E8':'#7A756E' }}>{icon}</div>
      <span style={{ fontSize:10, color:active?'#F5F0E8':'#7A756E', fontWeight:active?500:400 }}>{label}</span>
    </button>
  );
}
